...
import {
  LayoutDashboard, Calculator, BookOpen, FolderOpen, Settings, BarChart3,
  ShieldAlert, ChevronDown, TrendingUp, Flame, BarChart2, Percent, Search,
  Landmark, PieChart, Blocks, Menu, X, Scale, Activity, FileText, Gem, Wheat,
  Factory, Beef, UserCircle, Newspaper, Globe, Shield
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className={`hidden sm:flex lg:hidden fixed top-14 right-0 bottom-0 z-40 w-72 flex-col bg-card border-l border-border shadow-2xl transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] ${drawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          <div>
            ...
            {drawerCalcOpen && (
              <div className="ml-4 mt-1 space-y-0.5 border-l-2 border-border pl-3">
                {calcItems.map(({ path, icon: Icon, label, desc }) => (
                  <Link key={path} to={path} className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${isActive(path) ? 'text-primary dark:text-accent font-medium' : 'text-muted-foreground hover:text-foreground hover:bg-muted'}`}>
        ...
        <div className="border-t border-border px-4 py-3 space-y-1">
          ...
          <p className="text-[10px] text-muted-foreground">© {new Date().getFullYear()} Commodity Investor+</p>
        </div>
      </div>
