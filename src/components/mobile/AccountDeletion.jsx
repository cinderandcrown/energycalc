...
/**
 * Multi-step account deletion flow with typed confirmation.
 */
export default function AccountDeletion() {
  const [step, setStep] = useState(0); // 0=idle, 1=warning, 2=confirm type
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    // In a real scenario this would call a backend function to schedule deletion
    toast({
      title: "Account deletion requested",
      description: "Your account is scheduled for deletion. You will receive a confirmation email. Contact support@energycalc.app to cancel.",
    });
    setDeleting(false);
    setStep(0);
    setConfirmText("");
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Type <strong className="text-foreground font-mono">{CONFIRM_PHRASE}</strong> to confirm
          </p>
        </div>
      </div>
      <Input
